...
<div class="container" style="background-color:black; margin: 3%12%; margin-top:10%; padding:3%; opacity:0.7" >
         <h2 style="text-align:center; margin:20px; color:white;" >Past Events</h2>
  ...
  <input type="text" id="myInput" class="input-group" onkeyup="SearchEvent()" placeholder="Search for Event names.." title="Type in a name">
     ...
     <%
       try{
    	 ...
    	 //Query for event id
    	   PreparedStatement ps = con.prepareStatement("select * from event_tab where end_date < CURDATE();");
	  ...
	  <%
    	   while(rs.next()){
    		   ...
    		   <tr>
        		...
        		<td>
        			<button class="tab" style="background-color:black; color:white;" onclick="location.href='event_details2.jsp?event_id=<%=rs.getInt("event_id")%>'">View details</button>
        		</td>
        		</tr>
    		   <%
    	   }
